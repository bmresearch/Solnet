...
public class Transaction
{
    ...
    ///     <remarks>
    ///         These are typically created by invoking the <c>Build(IList{Account} signers)</c> method of the
    ///         <see cref="TransactionBuilder" />,
    ...
    {
        return Signatures.All(pair => pair.PublicKey.Verify(serializedMessage, pair.Signature));
    }
    ...
    {
        return VerifySignatures(CompileMessage());
    }
    ...
    ///     <remarks>
    ///         <para>
    ///             If the transaction <c>FeePayer</c> is not set, the first signer will be used as the transaction fee payer
    ///             account.
    ///         </para>
    ///         <para>
    ///             Transaction fields SHOULD NOT be modified after the first call to <c>Sign</c> or an externally created
    ///             signature
    ///             has been added to the transaction object, doing so will invalidate the signature and cause the transaction
    ///             to be
    ///             rejected by the cluster.
    ///         </para>
    ///         <para>
    ///             The transaction must have been assigned a valid <c>RecentBlockHash</c> or <c>NonceInformation</c> before
    ...
    public bool Sign(Account signer)
    {
        return Sign(new List<Account> {signer});
    }
    ...
    {
        PartialSign(new List<Account> {signer});
    }
    ...
    {
        return Add(new List<TransactionInstruction> {instruction});
    }
    ...
    {
        return Populate(Message.Deserialize(message), signatures);
    }
