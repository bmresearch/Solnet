    ...
    public class TransactionDecodingExample : IExample
    {
        ...
        public void Run()
        {
            ...
            byte[] msgData = new TransactionBuilder().SetRecentBlockHash(blockHash.Result.Value.Blockhash)
                .SetFeePayer(ownerAccount).AddInstruction(SystemProgram.CreateAccount(
                    ownerAccount,
                    mintAccount,
                    ...
                    initialAccount.PublicKey,
                    1_000_000,
                    ownerAccount)).AddInstruction(MemoProgram.NewMemo(initialAccount, "Hello from Sol.Net"))
                .CompileMessage();
            ...
            Console.WriteLine($"FeePayer: {tx.FeePayer.Key}");
            Console.WriteLine($"BlockHash/Nonce: {tx.RecentBlockHash}");
            foreach (SignaturePubKeyPair signaturePubKeyPair in tx.Signatures)
            {
                Console.WriteLine($"Signer: {signaturePubKeyPair.PublicKey.Key} \tSignature: {Encoders.Base58.EncodeData(signaturePubKeyPair.Signature)}");
            }
            foreach (TransactionInstruction txInstruction in tx.Instructions)
            {
                Console.WriteLine($"ProgramKey: {Encoders.Base58.EncodeData(txInstruction.ProgramId)}\n\tInstructionData: {Convert.ToBase64String(txInstruction.Data)}");
                foreach (AccountMeta accountMeta in txInstruction.Keys)
                {
                    Console.WriteLine($"\tAccountMeta: {accountMeta.PublicKey}\tWritable: {accountMeta.IsWritable}\tSigner: {accountMeta.IsSigner}");
                }
            }
